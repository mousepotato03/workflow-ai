-- Add hybrid search function that combines vector and text search
-- This function was referenced in the code but missing from the database
...
BEGIN
  ...
  WITH vector_search AS (
    SELECT 
      ...
      t.categories,
      t.is_active,
      (1 - (t.embedding <=> query_embedding_vector)) AS vector_sim
    FROM tools t
  ...
  SELECT 
    ...
    cr.categories,
    cr.is_active,
    (cr.vector_sim * vector_weight + cr.text_sim * text_weight) AS hybrid_score,
...
-- Create index for text similarity if not exists
...
-- Create a simple GIN index for categories array (without array_to_string)
CREATE INDEX IF NOT EXISTS idx_tools_categories_gin ON tools USING gin(categories);