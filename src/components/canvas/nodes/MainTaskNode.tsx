...
export const MainTaskNode: React.FC<MainTaskNodeProps> = ({
  ...
  const handleResize = useCallback(
    (event: any, data: { width: number; height: number }) => {
      setNodeSize({ width: data.width, height: data.height });
      onSizeUpdate?.(data.width, data.height);
    },
    [onSizeUpdate]
  );
  ...
  return (
    <>
      <NodeResizer
        ...
        handleClassName="w-8 h-8 bg-teal-400 border-2 border-white shadow-lg hover:bg-teal-500 hover:scale-110 transition-all cursor-nw-resize"
