...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    case "ADD_TOAST": {
      ...
      const duration = newToast.duration ?? DEFAULT_TOAST_DURATION;

      // Schedule auto dismiss if duration is not 0 (0 means persistent)
      if (duration > 0) {
        scheduleAutoDismiss(newToast.id, duration);
      }
    ...
    case "DISMISS_TOAST": {
      ...
      if (toastId && dismissTimeouts.has(toastId)) {
        clearTimeout(dismissTimeouts.get(toastId)!);
        dismissTimeouts.delete(toastId);
      }
...
function toast({ ...props }: Toast) {
  ...
  dispatch({
    ...
    toast: {
      ...
      id,
      open: true,
      // Remove onOpenChange to prevent immediate dismissal
      // onOpenChange: (open) => {
      //   console.log("토스트 onOpenChange:", { id, open });
      //   if (!open) {
      //     // Clear any pending auto-dismiss timeout when manually closed
