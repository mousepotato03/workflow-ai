...
export class DataPipeline {
  ...
  async transformToolData(
    rawData: any[]
  ): Promise<z.infer<typeof ToolDataSchema>[]> {
  ...
  private cleanToolData(rawItem: any): any {
    return {
      ...
      description: this.sanitizeString(
        rawItem.description || rawItem.tagline || ""
      ),
  ...
  private async enhanceToolData(
    tool: z.infer<typeof ToolDataSchema>
  ): Promise<z.infer<typeof ToolDataSchema>> {
  ...
  private async processBatch(
    ...
    const promises = batch.map(async (tool) => {
      try {
        ...
        const { error } = await this.supabase.from("tools").upsert(
          {
            name: tool.name,
            description: tool.description,
            url: tool.url,
  ...
  async validateDataQuality(): Promise<{
    ...
    const withEmbeddings = tools.filter(
      (tool) => tool.embedding && tool.embedding_text
    ).length;
  ...
  async runIncrementalUpdate(): Promise<{
    ...
    try {
      ...
      const newTools = transformedData.filter(
        (tool) => !existingTools.has(tool.name)
      );
      const updatedTools = transformedData.filter((tool) =>
        existingTools.has(tool.name)
      );
      ...
      if (updatedTools.length > 0) {
        ...
        results.errors.push(
          ...updateResults.errors.map((e) => ({
            source: "updated_tools",
            error: e.error,
          }))
        );
      }
