import { Node, Edge, NodeProps, NodeChange, EdgeChange, Connection } from 'reactflow';
 ...
 */
export interface CanvasNode<T extends CanvasNodeData = CanvasNodeData> extends Node<T> {
...
export interface CanvasActions {
  ...
  addNode: (nodeType: CanvasNodeType, position?: { x: number; y: number }, data?: Partial<CanvasNodeData>) => string;
...
export interface CanvasWorkflowIntegration {
  ...
  createNodeFromTask: (task: Task, position: { x: number; y: number }) => string;
...
export interface CanvasEventHandlers {
  ...
  onConnectStart?: (event: React.MouseEvent, handle: { nodeId: string; type: string; handleId?: string }) => void;
  ...
  onMove?: (event: React.MouseEvent, viewport: { x: number; y: number; zoom: number }) => void;
}

/**
 * Type guard functions for node data types.
 */
export const isMainTaskNodeData = (data: CanvasNodeData): data is MainTaskNodeData => {
  return 'goal' in data && 'isLocked' in data;
};

export const isSubtaskNodeData = (data: CanvasNodeData): data is SubtaskNodeData => {
  return 'taskDescription' in data && 'status' in data;
};

export const isGuideCardNodeData = (data: CanvasNodeData): data is GuideCardNodeData => {
  return 'title' in data && 'content' in data && 'isCollapsed' in data;
};

export const isShapeNodeData = (data: CanvasNodeData): data is ShapeNodeData => {
...
export namespace CanvasUtils {
  ...
  export const calculateNodePosition = (existingNodes: Node[], nodeSpacing: { x: number; y: number }): { x: number; y: number } => {
    ...
    while (positions.some(pos => Math.abs(pos.x - x) < nodeSpacing.x && Math.abs(pos.y - y) < nodeSpacing.y)) {
      x += nodeSpacing.x;
      if (x > 800) {
        x = 100;
        y += nodeSpacing.y;
  ...
  export const createDefaultNodeData = (type: CanvasNodeType): CanvasNodeData => {
  ...
  export const validateNodeData = (type: CanvasNodeType, data: CanvasNodeData): boolean => {
    switch (type) {
      case CanvasNodeType.MAIN_TASK:
      ...
      case CanvasNodeType.GUIDE_CARD:
        return isGuideCardNodeData(data) && Boolean(data.title?.trim()) && Boolean(data.associatedSubtaskId);
